...
TONE_ADDENDUM = (                        # ✏️ Build 4, step 4.1, intelligence lane
    "When a customer is abusive, threatening, or mentions legal action, acknowledge their concern once "
    "and immediately escalate to a human agent. Do not offer entitlements or continue the normal resolution flow."
)
...
def run_agent(pnr: str, last_name: str, message: str) -> str:            # ✏️ Build 1, step 1.2
    ...
    response = client.messages.create(
        model=MODEL, max_tokens=4096, system=[{"type": "text", "text": runtime_preamble() + SYSTEM_PROMPT + TONE_ADDENDUM, "cache_control": {"type": "ephemeral"}}],
