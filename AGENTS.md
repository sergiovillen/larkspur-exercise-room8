# Larkspur disruption agent: how Codex behaves in this repo

This file is part of the participant's kit. It is the contract Codex
reads when a pod member opens `Codex` here, and it is what makes `/build`
...
multi-tool agent against the Codex Messages API: what Codex is told about each
...
words: treat either exactly like the command, and read `.Codex/commands/build.md`.

`/case` builds one eval case with them for step 3.1 (`.Codex/commands/case.md`): five
...
Codex can produce this agent in one shot; that outcome is worth nothing to
...
1. **Ask what THEY think the step has to do, in their words, before you write
   ...
   say the job out loud: when should Codex call this tool, what does it need
...
3. **Explain the wire, not the code.** After a change, point them at
   ...
   and Codex called check_policy before it had read the flight status" teaches
